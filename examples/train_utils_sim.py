...
from jaxrl2.agents.fql.debug_utils import debug_pytree_shapes

#------------------------------------------------------ tools ------------------------------------------------------

def batch_size(observations):
    if isinstance(observations, dict) or hasattr(observations, 'get'):
        if 'pixels' in observations:
            bsz = int(observations['pixels'].shape[0])
        elif 'state' in observations:
            bsz = int(observations['state'].shape[0])
        else:
            first_key = list(observations.keys())[0]
            bsz = int(observations[first_key].shape[0])
    else:
        bsz = int(observations.shape[0])
...
def fql_trajwise_alternating_training_loop(variant, agent, env, eval_env, online_replay_buffer, replay_buffer, wandb_logger,
    ...
    with tqdm(total=variant.max_steps, initial=0) as pbar:
        while i <= variant.max_steps:
            traj = fql_collect_traj(variant, agent, env, i, agent_dp)
            ...
            if len(online_replay_buffer) > variant.start_online_updates:
                for _ in range(num_gradsteps):
                    ...
                    # debug_pytree_shapes("batch", batch, limit=50)
...
def fql_collect_traj(variant, agent, env, i, agent_dp=None):
    ...
    for t in tqdm(range(max_timesteps)):
        ...
        if t % query_frequency == 0:
            ...
            if i == 0:
                ...
                # shape references from agent: H (50), noise_chunk_shape (50,32)
                noise = jax.random.normal(key, (1, *agent.noise_chunk_shape))  
                noise_repeat = jax.numpy.repeat(noise[:, -1:, :], agent.H - noise.shape[1], axis=1)
                noise = jax.numpy.concatenate([noise, noise_repeat], axis=1)        # (1,50,32)
                actions_noise = noise[0, :agent.noise_chunk_shape[0], :]            # (50,32)
            else:
                # SAC agent predicts the noise for diffusion model (DSRL-NA style)
                actions_noise = agent.sample_actions(obs_dict)                       # (32,) or (50,32)
                actions_noise = np.reshape(actions_noise, agent.noise_chunk_shape)   
                # pad to H if needed
                pad = agent.H - actions_noise.shape[0]
                tail = np.repeat(actions_noise[-1:, :], pad, axis=0)                
                noise = jax.numpy.concatenate([actions_noise, tail], axis=0)[None]   

            # π0 inference
            actions_seq = agent_dp.infer(obs_pi_zero, noise=noise)["actions"]        # (50,14)

            # 버퍼용 시퀀스 저장 (batch 차원 제거)
            noise_seq = np.asarray(noise[0])            # (50,32)
            pi0_seq  = np.asarray(actions_seq)          # (50,14)
            pi0_noises_list.append(noise_seq)
            pi0_action_list.append(pi0_seq)
            obs_list.append(obs_dict)
            action_list.append(actions_noise)

        # 실제 env에 적용한 1-step 액션 (14,)
        action_t = pi0_seq[t % query_frequency]
        if 'libero' in variant.env:
    ...
    return {
        'observations': obs_list,              # list of dicts (Q+1)
        'actions': action_list,       # (Q, 32)
        'pi0_actions': pi0_action_list,        # (Q, 50, 14)
        'pi0_noises': pi0_noises_list,         # (Q, 50, 32)
        'rewards': rewards,                    # (Q,)
        'masks': masks,                        # (Q,)
...
def add_online_data_to_buffer(variant, traj, online_replay_buffer):
    ...
    actions = np.asarray(traj['actions'])             # (Q,32)
    rewards = np.asarray(traj['rewards'])             # (Q,)
    masks   = np.asarray(traj['masks'])               # (Q,)

    has_pi0 = ("pi0_actions" in traj) and len(traj["pi0_actions"]) > 0
    if has_pi0:
        pi0_actions = np.asarray(traj['pi0_actions']) # (Q,50,14)
        pi0_noises  = np.asarray(traj['pi0_noises'])  # (Q,50,32)
    ...
    for t in range(Q):
        ...
        # remove batch dimension added at collection time
        ...
        insert_dict = dict(
            ...
            actions=actions[t],                                   # (32,)
            next_actions=actions[t + 1] if t < Q - 1 else actions[t],  # (32,)
            rewards=rewards[t],
            masks=masks[t],
        ...
        if has_pi0:
            insert_dict.update(
                pi0_actions=pi0_actions[t],   # (50,14)
                pi0_noises=pi0_noises[t],     # (50,32)
            )
...
#     if "pi0_actions" in traj and traj["pi0_actions"]:
#         pi0_actions = np.array(traj['pi0_actions']) 
#         pi0_noises = np.array(traj['pi0_noises']) 
...
#                 next_observations=next_obs,
#                 actions=actions[t],
#                 pi0_actions = pi0_actions[t],
#                 pi0_noises = pi0_noises[t],
    ...
    for rollout_id in range(variant.eval_episodes):
        ...
        for t in tqdm(range(max_timesteps)):
            ...
            if t % query_frequency == 0:
                ...
                actions = agent_dp.infer(obs_pi_zero, noise=noise)["actions"]  #TODO
